...
function ArmyPage() {
  ...
  const duplicateUnitInStandby = (unitId) => {
    ...
    setArmyStandby({
      ...armyStandby,
      units: [...armyStandby.units, duplicatedUnit],
    });
  };
  ...
  const duplicateUnitInUnpainted = (unitId) => {
    ...
    setArmyUnpainted({
      ...armyUnpainted,
      units: [...armyUnpainted.units, duplicatedUnit],
    });
  };
  ...
  const moveUnpaintedUnitDown = (unitId) => {
    ...
    if (
      index < armyUnpainted.units.length - 1 &&
      armyUnpainted.units.length > 1
    ) {
      let newArmyUnpainted = armyUnpainted.units.slice();
      let placeHolder = newArmyUnpainted[index];
      newArmyUnpainted[index] = newArmyUnpainted[index + 1];
  ...
  const clearCurrentArmy = () => {
    let confirmed = window.confirm(
      "Are you sure you want delete your current army?"
    );
    if (confirmed) {
  ...
  const readyToStandby = (unitId) => {
    ...
    setArmyStandby({
      ...armyStandby,
      units: [...armyStandby.units, unitToBeMoved],
    });
  };
  ...
  const readyToUnpainted = (unitId) => {
    ...
    setArmyUnpainted({
      ...armyUnpainted,
      units: [...armyUnpainted.units, unitToBeMoved],
    });
  };
  ...
  return (
    <div className="border-black col-span-10 grid grid-cols-12 border-2 row-span-6">
      <ArmyLeft armyReady={armyReady} addUnitToArmy={addUnitToArmy} />
      <ArmyRight
        overLimit={overLimit}
